...
// Same sidebar-anim config read as serve.js, baked into manifest.json.
function readAnim(root) {
  try {
...
function run(root, outDir) {
  ...
  // 2 — manifest.json (tree with _order.json/_icons.json already applied)
  const manifest = { root: "", sep: "/", anim: readAnim(src), tree: buildTree(src) };
  ...
  if (!fs.existsSync(idx)) fs.writeFileSync(idx, SHELL);
